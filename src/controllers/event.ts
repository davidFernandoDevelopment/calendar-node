...
export const getEvents = async (req: Request, res: Response) => {
	const evts = await EventModel.find({}).populate('user', 'name');
...
export const updateEvent = async (req: Request, res: Response) => {
	...
	try {
		...
		if (evt.user.toString() !== uid)
			return res
				.status(401)
				.json({ ok: false, msg: 'No puedes editar el evento de otra persona' });

		const newEvt = {
			...req.body,
			user: uid,
		};
		const evtUpdated = await EventModel.findByIdAndUpdate(id, newEvt, {
			new: true,
		});
